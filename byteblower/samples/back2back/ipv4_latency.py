...
class Example:
    ...
    def run(self):
        ...
        # received frame.
        latency_trigger = self.port_2.RxLatencyBasicAdd()
        ...
        latency_trigger.FilterSet(bpf_filter)
        ...
        for iteration in range(1, int(duration_s)):
            ...
            latency = 0
            if last_interval_rx.PacketCountGet() != 0:
                latency = last_interval_rx.LatencyAverageGet()

            print("Tx {TX} frames, Rx {RX} frames, average latency {LATENCY}".format(TX=tx, RX=rx, LATENCY=latency))
